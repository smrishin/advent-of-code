...
def score_cookie(quantity):
    ...
    for i, item in enumerate(ingredients):
        ...
        calories += quantity[i] * item["calories"]
...
def make_cookies(remaining, total, quantity = []):
    ...
    for i in range(1, remaining + 1):
        ...
        if len(quantity) == len(ingredients) - 1: 
            ...
            curr_cookie_score, calories = score_cookie(quantity)
            if calories == 500 and curr_cookie_score > max_score:
...
# assert top_score_cookie('testinput.txt', 100) == 62842880
